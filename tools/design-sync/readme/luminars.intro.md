Luminars is a calm, warm, monochrome desktop product. It watches all day, so it must never
...
panel and the agent rail are **inset panels** — rounded surfaces (14px) floating on the canvas
with soft depth, never edge-to-edge sheets. Lists are calm cards and tables, not widget
dashboards; one primary object per screen, a readable content column.
