 ...
 * `bg-muted` at .6. It sits on `shadow-control`, the same small lift as the quiet verb beside it
 * (the sunken `search` variant stays flat). The field paints NO focus ring of its own — the global `:focus-visible`
...
const inputVariants = cva(
  "press-wide h-[34px] w-full min-w-0 rounded-sm border border-input bg-card px-2.5 py-[7px] text-13 text-foreground shadow-control duration-[120ms] file:inline-flex file:h-5 file:border-0 file:bg-transparent file:text-13 file:font-medium file:text-foreground placeholder:text-faint not-disabled:hover:border-border-strong disabled:cursor-default disabled:bg-muted disabled:opacity-60 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        default: "",
